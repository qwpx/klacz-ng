...
module KlaczNG.Ephemeral.Proto.GetValueResponse (GetValueResponse(..)) where
import Prelude ((+), (/))
...
data GetValueResponse = GetValueResponse{status :: !(P'.Maybe KlaczNG.Ephemeral.Proto.Status), status_info :: !(P'.Maybe P'.Utf8),
