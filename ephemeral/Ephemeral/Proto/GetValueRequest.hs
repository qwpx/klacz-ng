...
module Ephemeral.Proto.GetValueRequest (GetValueRequest(..)) where
import Prelude ((+), (/))
...
instance P'.ReflectDescriptor GetValueRequest where
  ...
  reflectDescriptorInfo _
   = Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".Ephemeral.Proto.GetValueRequest\", haskellPrefix = [], parentModule = [MName \"Ephemeral\",MName \"Proto\"], baseName = MName \"GetValueRequest\"}, descFilePath = [\"Ephemeral\",\"Proto\",\"GetValueRequest.hs\"], isGroup = False, fields = fromList [FieldInfo {fieldName = ProtoFName {protobufName' = FIName \".Ephemeral.Proto.GetValueRequest.key\", haskellPrefix' = [], parentModule' = [MName \"Ephemeral\",MName \"Proto\",MName \"GetValueRequest\"], baseName' = FName \"key\"}, fieldNumber = FieldId {getFieldId = 1}, wireTag = WireTag {getWireTag = 10}, packedTag = Nothing, wireTagLength = 1, isPacked = False, isRequired = False, canRepeat = False, mightPack = False, typeCode = FieldType {getFieldType = 9}, typeName = Nothing, hsRawDefault = Nothing, hsDefault = Nothing}], keys = fromList [], extRanges = [], knownKeys = fromList [], storeUnknown = False, lazyFields = False}"
